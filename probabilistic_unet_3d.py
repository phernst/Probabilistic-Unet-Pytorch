...
class Encoder(nn.Module):
    """
    A convolutional neural network, consisting of len(num_filters) times a block of
    no_convs_per_block convolutional layers, after each block a pooling operation is performed. And
    after each convolutional layer a non-linear (ReLU) activation function is applied.
    """

    def __init__(self, input_channels, num_classes, num_filters, no_convs_per_block, _initializers,
        ...
        for i in range(len(self.num_filters)):
            # Determine input_dim and output_dim of conv layers in this block. The first layer is
            # input x output, all the subsequent layers are output x output.
            input_dim = self.input_channels if i == 0 else output_dim
            output_dim = num_filters[i]

            if i != 0:
                layers.append(nn.AvgPool3d(
                    kernel_size=2, stride=2, padding=0, ceil_mode=True))

            layers.append(nn.Conv3d(input_dim, output_dim,
                          kernel_size=3, padding=int(padding)))
            layers.append(nn.ReLU(inplace=True))

            for _ in range(no_convs_per_block-1):
                layers.append(nn.Conv3d(output_dim, output_dim,
                              kernel_size=3, padding=int(padding)))
                layers.append(nn.ReLU(inplace=True))
    ...
    def forward(self, input_t):
        output = self.layers(input_t)
        return output


class AxisAlignedConvGaussian(nn.Module):
    """
    A convolutional net that parametrizes a Gaussian distribution with axis aligned covariance
    matrix.
    """
    ...
    def __init__(self, input_channels, num_classes, num_filters, no_convs_per_block, latent_dim,
        ...
        if self.posterior:
            ...
            self.name = 'Prior'
        self.encoder = Encoder(self.input_channels, num_classes, self.num_filters,
                               self.no_convs_per_block, initializers, posterior=self.posterior)
        self.conv_layer = nn.Conv3d(
            num_filters[-1], 2 * self.latent_dim, (1, 1, 1), stride=1)
        self.show_img = 0
        self.show_seg = 0
        self.show_concat = 0
        self.show_enc = 0
    ...
    def forward(self, input_t, segm=None):
        # If segmentation is not none, concatenate the mask to the channel axis of the input
        if segm is not None:
            self.show_img = input_t
            self.show_seg = segm
            input_t = torch.cat((input_t, segm), dim=1)
            self.show_concat = input_t
            self.sum_input = torch.sum(input_t)

        encoding = self.encoder(input_t)
        ...
        # We squeeze the second dimension twice, since otherwise it won't work when batch size
        # is equal to 1
...
class Fcomb(nn.Module):
    ...
    def __init__(self, num_filters, latent_dim, num_output_channels, num_classes, no_convs_fcomb,
        ...
        if self.use_tile:
            ...
            # Decoder of N x a 1x1 convolution followed by a ReLU activation function except for
            # the last layer
            ...
            for _ in range(no_convs_fcomb-2):
                layers.append(
                    nn.Conv3d(self.num_filters[0], self.num_filters[0], kernel_size=1))
                layers.append(nn.ReLU(inplace=True))
    ...
    def forward(self, feature_map, z):
        ...
        So broadcast Z to batch_sizexlatent_dimxHxWxD. Behavior is exactly the same as
        tf.tile (verified)
        """
        if self.use_tile:
            ...
            # Concatenate the feature map (output of the UNet) and the sample taken from the
            # latent space
...
class ProbabilisticUnet(nn.Module):
    ...
    def __init__(self, input_channels=1, num_classes=1, num_filters=None,
                 latent_dim=6, no_convs_fcomb=4, beta=10.0):
        super().__init__()
        self.input_channels = input_channels
        self.num_classes = num_classes
        self.num_filters = num_filters if num_filters is not None else [
            32, 64, 128, 192]
        self.latent_dim = latent_dim
        ...
        self.mean_reconstruction_loss = None
    ...
    def sample(self, testing=False):
        ...
        if not testing:
            ...
            # You can choose whether you mean a sample or the mean here. For the GED it is important
            # to take a sample.
    ...
    def reconstruct(self, use_posterior_mean=False, calculate_posterior=False, z_posterior=None):
        """
        Reconstruct a segmentation from a posterior sample (decoding a posterior sample) and
        UNet feature map
    ...
    def kl_divergence(self, analytic=True, calculate_posterior=False, z_posterior=None):
        ...
        calculate_posterior: if we use sampling to approximate KL we can sample
                             here or supply a sample
        """
        if analytic:
            # Need to add this to torch source code,
    ...
    def elbo(self, segm, analytic_kl=True, reconstruct_posterior_mean=False):
        ...
        return -(self.mean_reconstruction_loss + self.beta * self.kl_div)
