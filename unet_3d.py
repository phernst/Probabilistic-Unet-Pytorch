...
from unet_blocks_3d import DownConvBlock, UpConvBlock
...
class Unet(nn.Module):
    ...
    def __init__(self, input_channels, num_classes, num_filters, initializers,
        ...
        for i in range(len(self.num_filters)):
            input_t = self.input_channels if i == 0 else output_t
            output_t = self.num_filters[i]
            ...
            self.contracting_path.append(DownConvBlock(
                input_t, output_t, initializers, padding, pool=pool))

        self.upsampling_path = nn.ModuleList()

        n = len(self.num_filters) - 2
        for i in range(n, -1, -1):
            input_t = output_t + self.num_filters[i]
            output_t = self.num_filters[i]
            self.upsampling_path.append(UpConvBlock(
                input_t, output_t, initializers, padding))

        if self.apply_last_layer:
            self.last_layer = nn.Conv2d(output_t, num_classes, kernel_size=1)
